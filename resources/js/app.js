...
*/
import $ from 'jquery';
import 'popper.js'; // Necesario para Bootstrap 4 Modals
window.$ = $;
window.jQuery = $;
window.jquery = $;
jQuery = window.$;
...
class App {
  ...
  initComponents() {
      ...
      if ($(".side-nav").length) {
        ...
        $(".side-nav a").each(function () {
          ...
          if (this.href == pageUrl) {
            ...
            upperLevelParent.addClass("active");
          }
        });
    ...
    initCounterUp() {
      $('[data-plugin="counterup"]').each(function (idx, obj) {
        ...
        // Initialize counterUp with dynamic delay and time values
        $(this).counterUp({
          delay: delay, // Use the dynamic delay
          time: time, // Use the dynamic time
        });
      });
    }
  ...
  class ThemeCustomizer {
    ...
    _adjustLayout() {
      var self = this;
      var html = document.documentElement;  // Select the <html> element

      if (window.innerWidth <= 1199) {
        html.setAttribute("data-sidenav-size", "full");
      } else {
        html.setAttribute("data-sidenav-size", self.config.sidenav.size);
      }
    }
    ...
    setSwitchFromConfig() {
      ...
      // localStorage.setItem('__ADMINOX_CONFIG__', JSON.stringify(this.config));
