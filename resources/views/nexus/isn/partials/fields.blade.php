<form action="{{ isset($pago) ? route('isn.update', $pago->id) : route('isn.store') }}" method="POST">
    @csrf
    @if(isset($pago))
        @method('PUT')
    @endif
    ...
    <div class="col-md-6 pt-2">
        ...
        <select class="form-control" id="estatus" name="estatus" required>
            <option value="0" {{ (isset($pago) && $pago->estatus == 'pendiente') ? 'selected' : '' }}>Pendiente</option>
            <option value="1" {{ (isset($pago) && $pago->estatus == 'pagado') ? 'selected' : '' }}>Pagado</option>
        </select>
    </div>
